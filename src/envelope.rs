...
impl<CS: CipherSuite> Envelope<CS> {
    ...
    pub(crate) fn serialize(&self) -> Vec<u8> {
        [&self.nonce[..], &self.inner_envelope[..], &self.hmac[..]].concat()
    }
    ...
    pub(crate) fn dummy() -> Self {
        Self {
            ...
            inner_envelope: vec![0u8; <Key as SizedBytes>::Len::to_usize()],
    ...
    > {
        ...
        let (mode, ciphertext, client_s_pk) = (
    ...
    pub(crate) fn seal_raw(
        ...
        // FIXME call serialize_without_hmac on self perhaps?
        let mut hmac =
    ...
    pub fn as_byte_ptrs(&self) -> Vec<(*const u8, usize)> {
        vec![
            (self.inner_envelope.as_ptr(), self.inner_envelope.len()),
            (self.hmac.as_ptr(), self.hmac.len()),
        ]
    }
}
...
fn construct_aad(
    ...
    [server_s_pk.to_vec(), id_s, id_u].concat()
}
